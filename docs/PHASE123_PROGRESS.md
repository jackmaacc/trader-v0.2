...
## Independent book-accounting checkpoint

The [modeled-book auditor](PHASE3_ACCOUNTING.md) independently rebuilds each saved cash/quantity/receivable boundary from journal economics and corporate-action evidence, plus FIFO realized/unrealized attribution. It covers daily equities, crypto base/quote fees and dust, standard options, dividends and integer split basis. It does not invoke the portfolio transitions or infer source authenticity. Missing terminal marks suppress P&L metrics; overhead and frictionless expectancy remain separate requirements.

Ten focused tests passed and independent review confirmed the repair for unsupported option metadata. A read-only audit of both the previously retained synthetic database and its isolated restore under `artifacts/phase3_portfolio_drill_20260924T162550Z` independently matched all three cash/quantity/receivable boundaries, retaining identical head `f606b436beb9ed5746d04badd9e2ccc4b4ae5fabb15ff23f9d38a0bb082c0641`. Both correctly remained P&L-inconclusive without separately supplied terminal marks. Historical files were not changed.
