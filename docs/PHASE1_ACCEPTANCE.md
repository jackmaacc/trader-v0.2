...
- Local observer-ledger backup/restore evidence remains in `artifacts/operations_recovery`. The later four-file selected ownership/journal/seed/config restore in `artifacts/phase123_20260924T154714Z` verified saved bytes locally; the writer remained running, so it does not establish a consistent stopped-writer recovery point or off-host recovery.
...
| Backup/restore | Off-host protected backup, isolated restore and verified hashes; complete ownership/pending-order/config recovery scope | Local observer and selected ownership/journal/seed/config restores verified; complete stopped-writer scope and protected off-host restore pending |
